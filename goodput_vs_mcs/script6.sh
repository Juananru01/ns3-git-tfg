...
echo "MCS,CtlrRate,Mean_Goodput(Mbps),Distance(m),ChannelWidth(Hz),SimulationTime(s)" > goodput_vs_mcs_802.11be_5GHz_gi3200.csv
...
for channelWidth in "${channelWidths[@]}"; do
    for mcs in {0..13}; do
        ...
        output=$(./ns3 run "scratch/throughputvalue_vs_distance_802.11be --mcs=$mcs --channelWidth=$channelWidth --distance=3 --frequency=5 --simulationTime=35 --guard_interval=3200");
        ...
        echo "$mcs,$ctrlRate,$meanThroughput,$distance,$channelWidth,$simulationTime" >> goodput_vs_mcs_802.11be_5GHz_gi3200.csv
    done
done
