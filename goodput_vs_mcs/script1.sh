...
echo "MCS,CtlrRate,Mean_Goodput(Mbps),Distance(m),ChannelWidth(Hz),SimulationTime(s)" > goodput_vs_mcs_802.11ax_5GHz_gi800.csv
...
for channelWidth in "${channelWidths[@]}"; do
    for mcs in {0..11}; do
        ...
        echo "$mcs,$ctrlRate,$meanThroughput,$distance,$channelWidth,$simulationTime" >> goodput_vs_mcs_802.11ax_5GHz_gi800.csv
    done
done
