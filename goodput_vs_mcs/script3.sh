...
for channelWidth in "${channelWidths[@]}"; do
    for mcs in {0..11}; do
        ...
        output=$(./ns3 run "scratch/throughputvalue_vs_distance_802.11ax_v2 --mcs=$mcs --channelWidth=$channelWidth --distance=3 --frequency=5 --simulationTime=35 --guard_interval=800");
