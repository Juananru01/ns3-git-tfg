...
echo "MCS,CtlrRate,Mean_Goodput(Mbps),Distance(m),ChannelWidth(Hz),SimulationTime(s)" > goodput_vs_mcs_802.11ax_2.4GHz_gi800.csv
...
for channelWidth in "${channelWidths[@]}"; do
    for mcs in {0..11}; do
        ...
        output=$(./ns3 run "scratch/throughputvalue_vs_distance_802.11ax_v2 --mcs=$mcs --channelWidth=$channelWidth --distance=5 --frequency=5 --simulationTime=35 --guard_interval=800");
        ...
        echo "$mcs,$ctrlRate,$meanThroughput,$distance,$channelWidth,$simulationTime" >> goodput_vs_mcs_802.11ax_2.4GHz_gi800.csv
    done
done
