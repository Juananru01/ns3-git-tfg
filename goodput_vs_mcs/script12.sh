...
echo "MCS,Mean_Goodput(Mbps),Distance(m),ChannelWidth(Hz),SimulationTime(s)" > goodput_vs_mcs_802.11be_6GHz_gi3200.csv
...
for channelWidth in "${channelWidths[@]}"; do
    for mcs in {0..13}; do
        ...
        output=$(./ns3 run "scratch/throughputvalue_vs_distance_802.11be --mcs=$mcs --channelWidth=$channelWidth --distance=3 --frequency=6 --simulationTime=35 --guard_interval=3200");
        ...
        echo "$mcs,$meanThroughput,$distance,$channelWidth,$simulationTime" >> goodput_vs_mcs_802.11be_6GHz_gi3200.csv
    done
done
