...
echo "Mean_Goodput(Mbps),PayloadSize(bytes),Distance(m),FrequencyBand(GHz),ChannelWidth(Hz),SimulationTime(s),Seed" > goodput_vs_payloadsize_802.11ax_maxChannelWidths_gi800_seed$seed.csv
...
for payloadSize in {200..10000..200}; do
    ...
    echo "$meanThroughput,$payloadSize,$distance,$frequencyBand,$maxChannelWidth,$simulationTime,$seed" >> goodput_vs_payloadsize_802.11ax_maxChannelWidths_gi800_seed$seed.csv
done
