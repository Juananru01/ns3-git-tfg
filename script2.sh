...
echo "MCS,CtlrRate,Mean_Throughput(Mbps),Distance(m),SimulationTime(s)" > resultados_802.11ax_v2_2.4GHz_channelWidth40MHz_gi800.csv
...
for mcs in {0..11}; do
    for distance in {0..100..5}; do
        ...
        output=$(./ns3 run "scratch/throughputvalue_vs_distance_802.11ax_v2 --mcs=$mcs --distance=$distance --frequency=2.4 --simulationTime=25 --channelWidth=40 --guard_interval=800");
        ...
        echo "$mcs,$ctrlRate,$meanThroughput,$distance,$simulationTime" >> resultados_802.11ax_v2_2.4GHz_channelWidth40MHz_gi800.csv
    done
done
