...
echo "QueueDropPackets,PacketsInQueue,PacketsDropPhyRx,PacketsDropPhyRxInterferences,Time(s),NumSTAs,FrequencyBand(GHz),ChannelWidth(MHz),Seed,TotalSimulationTime(s)" > dropPackets_vs_tiempo_802.11be_maxWidthChannels_$frequencyBand'GHz'_gi800_seed$seed.csv
...
for time in $(seq 0 $simulationTime); do
    ...
    if [ -n "$line" ]; then
        ...
        if [[ -n $dropPhyRx && -n $dropPhyRxInterferences && -n $packetsInQueue && -n $queueDropPackets ]]; then
            ...
            echo "$output_line" >> dropPackets_vs_tiempo_802.11be_maxWidthChannels_${frequencyBand}GHz_gi800_seed${seed}.csv
        fi
    fi
done
