...
for df in dataframes2:
    if mean_df2.empty:
        mean_df2 = df.copy()
        mean_df2['QueueDropPackets'] = 0
    else:
        if not df['Time(s)'].equals(mean_df2['Time(s)']) or not df['FrequencyBand(GHz)'].equals(mean_df2['FrequencyBand(GHz)']):
            raise ValueError("Las filas de los archivos CSV no coinciden exactamente.")

# Calcular la media de 'QueueDropPackets' para cada fila
mean_df['QueueDropPackets'] = sum(df['QueueDropPackets'] for df in dataframes) / len(dataframes)
mean_df2['QueueDropPackets'] = sum(df['QueueDropPackets'] for df in dataframes2) / len(dataframes2)

# Calcular la desviación estándar de 'Mean_Goodput(Mbps)' para cada fila
std_dev = np.std([df['QueueDropPackets'] for df in dataframes], axis=0, ddof=1)
mean_df['Std_QueueDropPackets'] = std_dev

std_dev2 = np.std([df['QueueDropPackets'] for df in dataframes2], axis=0, ddof=1)
mean_df2['Std_QueueDropPackets'] = std_dev2
...
mean_df['CI_Lower'] = mean_df['QueueDropPackets'] - z_score * mean_df['Std_QueueDropPackets'] / np.sqrt(len(dataframes))
mean_df['CI_Upper'] = mean_df['QueueDropPackets'] + z_score * mean_df['Std_QueueDropPackets'] / np.sqrt(len(dataframes))
mean_df2['CI_Lower'] = mean_df2['QueueDropPackets'] - z_score * mean_df2['Std_QueueDropPackets'] / np.sqrt(len(dataframes2))
mean_df2['CI_Upper'] = mean_df2['QueueDropPackets'] + z_score * mean_df2['Std_QueueDropPackets'] / np.sqrt(len(dataframes2))
...
for frequencyBand, group in mean_df.groupby('FrequencyBand(GHz)'):
    ...
    plt.errorbar(group['Time(s)'], group['QueueDropPackets'], 
                 yerr=[group['QueueDropPackets'] - group['CI_Lower'], group['CI_Upper'] - group['QueueDropPackets']], 
...
plt.ylabel('Mean QueueDropPackets', fontsize=18)
...
plt.yticks(np.arange(0, 120000000 + 1, 10000000), fontsize=17)
