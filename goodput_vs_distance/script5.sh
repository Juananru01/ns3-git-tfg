...
echo "MCS,CtlrRate,Mean_Throughput(Mbps),Distance(m),SimulationTime(s)" > resultados_802.11be_5GHz_channelWidth160MHz_gi800.csv
...
for mcs in {0..13}; do
    for distance in {0..100..5}; do
        ...
        output=$(./ns3 run "scratch/throughputvalue_vs_distance_802.11be --mcs=$mcs --distance=$distance --frequency=5 --simulationTime=25 --channelWidth=160 --guard_interval=800");
        ...
        echo "$mcs,$ctrlRate,$meanThroughput,$distance,$simulationTime" >> resultados_802.11be_5GHz_channelWidth160MHz_gi800.csv
    done
done
