...
echo "MCS,Mean_Throughput(Mbps),Distance(m),SimulationTime(s),Seed" > resultados_802.11ax_v2_6GHz_channelWidth160MHz_gi800_seed$seed.csv
...
for mcs in {0..11}; do
    for distance in {0..100..5}; do
        ...
        output=$(../ns3 run "scratch/throughputvalue_vs_distance_802.11ax_v2 --mcs=$mcs --distance=$distance --frequency=6 --simulationTime=25 --channelWidth=160 --guard_interval=800 --seed=$seed");
        ...
        echo "$mcs,$meanThroughput,$distance,$simulationTime,$seed" >> resultados_802.11ax_v2_6GHz_channelWidth160MHz_gi800_seed$seed.csv
    done
done
