...
echo "PacketsInQueue,Time(ms),NumSTAs,FrequencyBand(GHz),ChannelWidth(MHz),Seed,TotalSimulationTime(s)" > queuePackets_vs_tiempo_802.11be_maxWidthChannels_$frequencyBand'GHz'_gi800_seed$seed.csv
...
echo "$output" | while IFS=', ' read -r line; do
    ...
    echo "$packets,$time,$numStations,$frequencyBand,$maxChannelWidth,$seed,$simulationTime" >> queuePackets_vs_tiempo_802.11be_maxWidthChannels_${frequencyBand}GHz_gi800_seed${seed}.csv
done