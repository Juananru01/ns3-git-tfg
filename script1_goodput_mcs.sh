...
echo "MCS,CtlrRate,Mean_Goodput(Mbps),Distance(m),ChannelWidth(Hz),GI(ns),SimulationTime(s),Seed" > goodput_vs_mcs_802.11ax_5GHz_seed$seed.csv
...
for gi in "${gis[@]}"; do
    for channelWidth in "${channelWidths[@]}"; do
        for mcs in {0..11}; do
            ...
            output=$(../ns3 run "scratch/throughputvalue_vs_distance_802.11ax_v2 --mcs=$mcs --channelWidth=$channelWidth --distance=3 --frequency=5 --simulationTime=35 --guard_interval=$gi --seed=$seed");
            ...
            echo "$mcs,$ctrlRate,$meanThroughput,$distance,$channelWidth,$gi,$simulationTime,$seed" >> goodput_vs_mcs_802.11ax_5GHz_seed$seed.csv
        done
    done
done