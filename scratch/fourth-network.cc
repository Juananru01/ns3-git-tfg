...
std::vector<Ptr<PacketSink>> sinks; // Vector para almacenar los receptores de paquetes
...
void CalculateThroughput()
{
    ...
    //for (size_t i = 0; i < 1; ++i)
    {
        ...
        std::cout << "STA " << i + 1 << ": Total bytes received: " << totalBytes << ", Total time: " << totalTime << "s, Throughput: " << throughput << " Mbps" << std::endl;
    }
}
...
int main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 0; i < numStations; ++i)
    {
        positionAlloc->Add(Vector(10.0 + (i * 5), 0.0, 0.0)); // STAs distribuidas en línea
    }
    ...
    sinks.resize(numStations, StaticCast<PacketSink>(sinkApps.Get(0)));

    sinkApps.Start(Seconds(0.0));

    /* Install UDP Transmitters on the stations */
    for (uint32_t i = 0; i < numStations; ++i)
    {
