...
echo "Mean_Goodput(Mbps),PayloadSize(bytes),Distance(m),FrequencyBand(GHz),ChannelWidth(Hz),SimulationTime(s)" > goodput_vs_payloadsize_802.11be_gi800.csv
...
for frequencyBand in "${frequencyBands[@]}"; do
    for payloadSize in {200..10000..200}; do
        ...
        output=$(./ns3 run "scratch/goodput_vs_payloadsize_802.11be_minstrel --payloadSize=$payloadSize --channelWidth=$channelWidth --distance=3 --frequency=$frequencyBand --simulationTime=25 --guard_interval=800");
        ...
        echo "$meanThroughput,$payloadSize,$distance,$frequencyBand,$channelWidth,$simulationTime" >> goodput_vs_payloadsize_802.11be_gi800.csv
    done
done
